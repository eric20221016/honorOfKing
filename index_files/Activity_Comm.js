...
function js_htmlspecialchars_decode(html) {
	...
	html = html.replace(/&quot;/gi, '"');
	html = html.replace(/&#039;/gi, '\'');
	html = html.replace(/&lt;([\/]?(p|img|span|strong|br|h\d|div|table|source|tbody|audio|object|thead|tfoot|tr|th|td|dd|dt|dl|ul|li|ol|a|embed))/gi, "<$1");
	html = html.replace(/&gt;/gi, '>');
...
//moss-发布时间
function ReloadPubdateMoss(string) {
	var yearStr=string.slice(0,10);
	var hourStr=string.slice(11,19)
	var newStr=yearStr+'&'+hourStr;
	var re = /^(\d{2,4})-(\d{1,2})-(\d{1,2})&(\d{1,2}):(\d{1,2}):(\d{1,2})$/;
	if (re.test(newStr)) {
		var t = newStr.match(re);
		var d = new Date(t[1], t[2] - 1, t[3], t[4], t[5], t[6]);
		var c = new Date();
		var s = (c.getTime() - d.getTime()) / 1000;
		var m = Math.floor(s / 60);
...
} /*  |xGv00|8fa6a66cf4c35a448b6533e6aba97d84 */