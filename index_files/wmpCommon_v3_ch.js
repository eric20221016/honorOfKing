...
function uploadTotalPlay(id, Type, iFrom, sControl, fromDetail, action_type, ext) {
    fromDetail = 1;
    if ((iFrom == 1 || iFrom == 0) && !sControl) {
        var timer = setInterval(function () {
            if (LWPlayer.getPlaytime && LWPlayer.getPlaytime() > 0) {
    ...
    typeInfo.then(function (data) {
        ...
        $.each(data.msg, function (i, v) {
            if (i.split('_')[1] != 0) {
                var p = null;
                if (G_biz == '18') {
                    p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_VIDEOLIST_GW_CH_' + G_biz + '_' + i.split('_')[1] + '.js', 'resObj_' + i.split('_')[1]);
                } else {
                    p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_VIDEOLIST_GW_' + G_biz + '_' + i.split('_')[1] + '.js', 'resObj_' + i.split('_')[1]);
                }
                promisearr.push(p);
            }
        });
    ...
    typeInfo.then(function (data) {
        ...
        $.each(data.msg, function (i, v) {
            if (i.split('_')[1] != 0) {
                var p = null;
                if (G_biz == '18') {
                    p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_NEWSLIST_GW_CH_' + G_biz + '_' + i.split('_')[1] + '.js', 'newsResObj_' + i.split('_')[1]);
                } else {
                    p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_NEWSLIST_GW_' + G_biz + '_' + i.split('_')[1] + '.js', 'newsResObj_' + i.split('_')[1]);
                }
                promisearr.push(p);
            }
        });
...
function QueryNewList(callback) {
    var p = null;
    if (G_biz == '18') {
        p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_RANKLIST_GW_CH_' + G_biz + '.js', 'rankObj');
    } else {
        p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_RANKLIST_GW_' + G_biz + '.js', 'rankObj');
    }
    p.then(function (data) {
...
function QueryNewsNewList(callback) {
    var p = null;
    if (G_biz == '18') {
        p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_NEWS_RANKLIST_GW_CH_' + G_biz + '.js', 'newsRankObj');
    } else {
        p = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/v3/WMP_NEWS_RANKLIST_GW_' + G_biz + '.js', 'newsRankObj');
    }
...
function QueryDetailInfo(id, callback) {
    var BizInfo = postScriptDeferStaticData('//gicp.qq.com/wmp/data/js/PAGE_WMP_BIZ_INFO.js', 'BizInfo');
...
function CreateCommentIframe(ElementId, source) {
    ...
    iframe.src = '//page.coral.qq.com/coralpage/comment/coral.html';
...
function GetQVideoPlay(vid, ElementId, iFrom, url, bullet, noautoplay) {
    ...
    if (iFrom == 1 || iFrom == 0) {
        ...
        loadScript('//vm.gtimg.cn/tencentvideo/txp/js/txplayer.js', function () {
            ...
            window.LWPlayer.getPlaytime = function () {
                return window.LWPlayer.getCurrentTime();
            }
